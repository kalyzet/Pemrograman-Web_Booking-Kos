    ...
    <form action="{{ route('booking', $boardingHouse->slug) }}" method="POST" class="relative flex flex-col gap-4 mt-5">
        ...
        <div id="RoomsContainer" class="flex flex-col gap-4 px-5">
            @foreach ($boardingHouse->rooms as $room)
                <label class="relative group">
                    ...
                    <div
                        class="flex rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white hover:border-[#91e2eb] group-has-[:checked]:ring-2 group-has-[:checked]:ring-[#91e2eb] transition-all duration-300">
                        <div class="flex w-[120px] h-[156px] shrink-0 rounded-[30px] bg-[#D9D9D9] overflow-hidden">
                            @foreach ($room->roomImages as $img)
                                <img src="{{ asset('storage/' . $img->image) }}" class="w-full h-auto">
                            @endforeach
                        </div>
                        <div class="flex flex-col gap-3 w-full">
        ...
        <div id="BottomButton" class="relative flex w-full h-[98px] shrink-0">
            <div class="fixed bottom-[30px] w-full max-w-[640px] px-5 z-10">
                <button class="w-full rounded-full p-[14px_20px] font-bold text-white text-center"
                    style="background-color: rgb(22, 208, 228);">Lanjut
                    Booking</button>
            </div>
        </div>
    </form>
